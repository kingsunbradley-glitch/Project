...
# --- 4. 261Rf Data (New Section) ---
# NOTE: Fill your data in the arrays below.
rf_en_present = np.array([])     # <--- Add your energy data here
rf_en_dubna = np.array([])           # <--- Add your energy data here
rf_en_riken = np.array([])           # <--- Add your energy data here
rf_en_gsi = np.array([8.52])             # <--- Add your energy data here

# Note: Time unit should be converted to ms if not already.
rf_hist_present = np.array([5.297,7.24]) * 1000.0   # <--- Add your time data here
rf_hist_dubna = np.array([3.0324,0.3923]) * 1000.0                  # <--- Add your time data here
rf_hist_riken = np.array([2.97,8.3,3.73])  * 1000.0                 # <--- Add your time data here
rf_hist_gsi = np.array([4.7,14.5])   * 1000.0                  # <--- Add your time data here

# Collect all time data for fitting
rf_fit_data = np.concatenate([rf_hist_present, rf_hist_dubna, rf_hist_riken, rf_hist_gsi])
...
# --- Plotting Main Logic ---
# -----------------------------------------------------------------
# Adjusted figsize for 4 rows
fig = plt.figure(figsize=(12, 14)) 
num_rows = len(all_datasets)
gs = GridSpec(num_rows, 2, figure=fig, wspace=0, hspace=0, width_ratios=[2, 1])
...
for i, data in enumerate(all_datasets):
    ...
    # Hide X labels for top rows (Only show for the last one)
    if i < num_rows - 1:
    ...
    # -------------------------------------------------------------
    # [FIX] Set Y-Limits AFTER plotting both Energy and Time
    # -------------------------------------------------------------
    # This ensures the Y-axis scales to fit the TALLEST bar from EITHER plot
    ax_left.set_ylim(bottom=0)
    current_ylim = ax_left.get_ylim() # Matplotlib auto-updates this when ax_right plots
