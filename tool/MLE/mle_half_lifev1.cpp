...
//
// *** 修改版 ***
// 输入数据已硬编码在 main 函数的开头。
// 请在 main 函数内的 "++++++++++ 在这里修改输入数据 ++++++++++" 部分修改数据。
//
// 编译:
...
int main(int argc, char **argv) {
    ...
    // ++++++++++ 在这里修改输入数据 ++++++++++
    // 直接在此处声明 C++ vector 来存储衰变时间 (单位: s)
    //
    // 示例数据 (来自原代码注释):
    std::vector<double> times = {358.126, 1.1811, 761.6, 1.6350, 967.49, 805.50};
    //
    // 示例2 (空数据，用于测试):
    ...
    // 检查数据是否为空
    if (times.empty()) {
        std::cerr << "No times provided (in-code vector is empty). Exiting.\n";
        return 1;
    }
