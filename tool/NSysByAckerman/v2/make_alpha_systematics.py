...
def read_livechart_csv(url: str = LIVECHART_URL, retries: int = 4, sleep_s: float = 2.0) -> pd.DataFrame:
    """Download LiveChart CSV with a User-Agent and a few retries."""
    last_err = None
    ...
    raise RuntimeError(f"Failed to download LiveChart data after {retries} attempts: {last_err}")
