...
def plot_qalpha(ax, df, show_ylabel=True, legend=True):
    ...
    if len(exp):
        ax.errorbar(
            exp['N'], exp['EXP'] / 1000.0, yerr=exp['error'] / 1000.0,
            fmt='o', color='red', markersize=4, label='EXP', zorder=3
        )

    vals = []
    for s in [df['UNEDF1'], df['WS4+RBF'], exp['EXP'] / 1000.0 if len(exp) else pd.Series(dtype=float)]:
...
def plot_delta2(ax, df, show_ylabel=True, legend=True):
    ...
    if len(exp):
        ax.errorbar(
            ...
            fmt='o', color='red', markersize=4, label='idx=1', zorder=3
        )

    vals = pd.concat([
        all_valid['delta2'] if len(all_valid) else pd.Series(dtype=float),
...
def make_combined_plot(file_left: str, file_right: str, show=True):
    ...
    out_png = f'{label_left}_{label_right}_combined_triple.png'
    out_pdf = f'{label_left}_{label_right}_combined_triple.pdf'
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('用法: python CombinedTriplePlotDecayPro.py dataHsPro.dat dataDsPro.dat')
        print('或  : python CombinedTriplePlotDecayPro.py dataHsPro.dat dataDsPro.dat --no-show')
        sys.exit(1)
