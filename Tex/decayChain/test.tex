...
\begin{tikzpicture}
    ...
    \tikzset{
        ...
        reaction/.style    = {font=\huge, align=center, text width=8cm},
    ...
    \node[font=\large, above=0.1cm of Inj1] {EVR};
    \draw[decay] (Inj1) -- (Ds1);
    % --- MODIFICATION END ---
    ...
    \draw[decay] (Rf1.south) -- ++(0.4, -\sfbranchlen);
    \node[sfinfo, below=\sfbranchlen+0.2cm of Rf1.south] {SF \\ 157.3 ~MeV\\(83.6+73.7~MeV) \\ 5.297 s};
    % --- Chain 2 (IMP 2) ---
    ...
    \node[font=\large, above=0.1cm of Inj2] {EVR};
    \draw[decay] (Inj2) -- (Ds2);
    % --- MODIFICATION END ---
    ...
    \draw[decay] (Rf2.south) -- ++(0.4, -\sfbranchlen);
    \node[sfinfo, below=\sfbranchlen+0.2cm of Rf2.south] {SF \\ 191.2 ~MeV\\(131.6+58.6~MeV) \\ 7.24 s};
    ...
    \node[reaction, anchor=north west] at ($(title1.north west) + (-12cm, -2cm)$) {
        Reaction Channel ${}^{238}\text{U}({}^{40}\text{Ar}, x~ n)^{278-x}\text{Ds}$
    };
\end{tikzpicture}
\end{document}