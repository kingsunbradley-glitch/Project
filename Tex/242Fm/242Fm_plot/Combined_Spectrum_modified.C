{
    ...
    int fontCode = 133;
    double commonTextSize = 44.0; // One size larger for all text except (a), (b)
    double panelLabelSize = 48.0; // Keep panel labels at their original size
    ...
    double xTitleOffset = 1.35;
    double yTitleOffset = 1.05;

    // Position of the common Y title on the whole canvas.
    // A smaller X value moves it farther away from the plotting frame.
    double commonYTitleX = 0.015;

    // ------------------------------------------------------------
    // Panel (b): SF annotation and horizontal brace
    // ------------------------------------------------------------
    double sfRegionXMin = 105.0;
    double sfRegionXMax = 202.0;
    double sfLabelX = 155.0;
    double sfLabelY = 3.90;
    double sfBraceShoulderY = 1.69;
    double sfBraceEndY = 1.48;
    double sfBraceCenterY = 2.17;
    ...
    // ============================================================
    // PowerPoint-style horizontal brace : panel (b)
    //
    // The brace spans 105--202 MeV.
    // It indicates
    // that the events above
    // 100 MeV are the spontaneous-fission events of 242Fm.
    // Six cubic Bezier segments give the ends a smooth hooked shape and
    // preserve a pointed center.  Y coordinates are constructed in log10(Y)
    // so the brace retains its intended shape on this logarithmic pad.
    // ============================================================

    const int braceSamplesPerSegment = 12;
    const int nBraceSegments = 6;
    const int nBracePoints =
        nBraceSegments * braceSamplesPerSegment + 1;

    double sfBraceX[nBracePoints];
    double sfBraceY[nBracePoints];

    double logBraceEndY =
        TMath::Log10(sfBraceEndY);

    double logBraceShoulderY =
        TMath::Log10(sfBraceShoulderY);

    double logBraceCenterY =
        TMath::Log10(sfBraceCenterY);

    int bracePointIndex = 0;

    auto AddBezierSegment =
        [&](double x0, double y0,
            double x1, double y1,
            double x2, double y2,
            double x3, double y3)
        {
            int firstSample =
                (bracePointIndex == 0) ? 0 : 1;

            for (
                int sample = firstSample;
                sample <= braceSamplesPerSegment;
                ++sample
            )
            {
                double u =
                    ...
                    /
                    static_cast<double>(braceSamplesPerSegment);

                double oneMinusU = 1.0 - u;

                sfBraceX[bracePointIndex] =
                    oneMinusU * oneMinusU * oneMinusU * x0
                    + 3.0 * oneMinusU * oneMinusU * u * x1
                    + 3.0 * oneMinusU * u * u * x2
                    + u * u * u * x3;

                double logY =
                    oneMinusU * oneMinusU * oneMinusU * y0
                    + 3.0 * oneMinusU * oneMinusU * u * y1
                    + 3.0 * oneMinusU * u * u * y2
                    + u * u * u * y3;

                sfBraceY[bracePointIndex] =
                    TMath::Power(10.0, logY);
    ...
    // Left hooked end.
    AddBezierSegment(
        sfRegionXMin,     logBraceEndY,
        sfRegionXMin + 4, logBraceEndY,
        sfRegionXMin + 2, logBraceShoulderY,
        sfRegionXMin + 11, logBraceShoulderY
    );

    // Left horizontal body.
    AddBezierSegment(
        sfRegionXMin + 11, logBraceShoulderY,
        sfRegionXMin + 20, logBraceShoulderY,
        sfLabelX - 16,     logBraceShoulderY,
        sfLabelX - 9,      logBraceShoulderY
    );

    // Left side of the pointed center.
    AddBezierSegment(
        sfLabelX - 9, logBraceShoulderY,
        sfLabelX - 4, logBraceShoulderY,
        sfLabelX - 3, logBraceShoulderY,
        sfLabelX,     logBraceCenterY
    );

    // Right side of the pointed center.
    AddBezierSegment(
        sfLabelX,     logBraceCenterY,
        sfLabelX + 3, logBraceShoulderY,
        sfLabelX + 4, logBraceShoulderY,
    ...
    AddBezierSegment(
        sfLabelX + 9,       logBraceShoulderY,
        sfLabelX + 16,      logBraceShoulderY,
        sfRegionXMax - 20,  logBraceShoulderY,
        sfRegionXMax - 11,  logBraceShoulderY
    );

    // Right hooked end.
    AddBezierSegment(
        sfRegionXMax - 11, logBraceShoulderY,
        sfRegionXMax - 2,  logBraceShoulderY,
        sfRegionXMax - 4,  logBraceEndY,
        sfRegionXMax,      logBraceEndY
    );

    TGraph *sfBrace = new TGraph(
        nBracePoints,
        sfBraceX,
        sfBraceY
    );

    sfBrace->SetName("sfRegionBrace");
    sfBrace->SetLineColor(kBlack);
    sfBrace->SetLineWidth(histLineWidth);
    sfBrace->Draw("L same");
