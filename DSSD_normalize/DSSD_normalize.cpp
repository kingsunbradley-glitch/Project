...
ErrorCode DSSD_Normalize_Process() {
    ...
    TString filter_str = TString::Format(
        "DSSDX_mul == 1 && DSSDY_mul == 1  && Veto_mul==0 && SSD_mul==0" // implant event and deacy event with full energy selection
    ...
    std::cout << "    Optimal reference for X-side normalization: Y_Ch = " << best_Y_Normalize_Ch << std::endl;
      ...
      // 【新建议】保存用于选择参考道的柱状图
    std::cout << "    Writing reference channel histograms to diagnostic file..." << std::endl;
    outFile->cd();
    h_x_hits->Write();
    h_y_hits->Write();
    ...
    txt_out << std::fixed << std::setprecision(3);

    // 设置浮点数的输出格式
    // std::fixed -> 使用定点表示法 (例如 1.234 而不是 1.23e+00)
    // std::setprecision(3) -> 小数点后显示3位
