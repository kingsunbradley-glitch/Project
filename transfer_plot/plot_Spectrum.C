 ...
 * 1. 链接数据 (TChain)
    ...
    估计并扣除本底 (TSpectrum::Background)

    克隆和相减 (TH1::Clone, TH1::Add)

    自动寻峰 (TSpectrum::Search)

    循环拟合 (Loop + TH1::Fit)

    结果输出 (std::cout 和 std::ofstream)                                                *
...
void analyze_and_find_peaks( ) {
    ...
    */
    ...
    tr_map->Add("/home/evalie2/Project/document/273Ds/inter_map/SS03200075_map.root"); // <- 修正
    tr_map->Add("/home/evalie2/Project/document/273Ds/inter_map/SS03200080_map.root"); // <- 修正
    tr_map->Add("/home/evalie2/Project/document/273Ds/inter_map/SS03200115_map.root"); // <- 修正
    */
    ...
    for (int i = 75; i <= 182; ++i) {
        tr_map->Add(TString::Format("%s%s%05d_map.root", dataPath, "SS032", i));
    }   
    ...
    TH1 *h_bkg = s->Background(h1, 20, "");// 20 means the smoothing parameter,if too small, the bkg will occpupy the peaks
    ...
    outfile << "Peak_ID\tEnergy_Mean\tEnergy_Error\tSigma\tSigma_Error\tFWHM\tFWHM_Error" << std::endl;
