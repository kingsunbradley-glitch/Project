...
void draw_single_plot_DSSD_time(TTree* tree) {
...
TString hist_name = Form("DSSD_tdiff");

  

// 动态生成筛选条件

TString cut_condition = Form("DSSDX_mul==1 && DSSDY_mul==1 && abs(DSSDX_E[0]-DSSDY_E[0])<500");

  

// 构建完整的Draw命令

TString draw_command = Form("DSSDX_Ts[0]-DSSDY_Ts[0]>>%s(100,-500,500)", hist_name.Data());

  

// 创建一个新的画布来显示新图

TString canvas_name = Form("T_DSSDX_vs_DSSDY");

TString canvas_title = Form("Time Difference for DSSDX vs DSSDY");
...
draw_single_plot_DSSD_time(tr_map);
